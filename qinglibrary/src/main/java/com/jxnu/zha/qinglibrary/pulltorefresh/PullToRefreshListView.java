package com.jxnu.zha.qinglibrary.pulltorefresh;
...
public class PullToRefreshListView extends PullToRefreshBase<ListView>
	 ...
	 * 没有数据时的提示信息
	 */
	LinearLayout tipsView = null;
	/**
	 * 设置没有内容时的消息提示信息
	 * @author DaiQing.Zha
	 * @date 2016-03-02
	 * @param tipsTxt 要提示的消息文本
	 */
	public void setTipsContent(String tipsTxt){
		if (mListView != null && tipsView == null) {

			tipsView = (LinearLayout) LayoutInflater.from(getContext())
					.inflate(R.layout.pull_to_refresh_tips_content, null);
			((TextView)tipsView.findViewById(R.id.tip_no_data)).setText(tipsTxt);
			LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT, LinearLayout.LayoutParams.FILL_PARENT);
			tipsView.setLayoutParams(params);
			tipsView.setGravity(Gravity.CENTER_HORIZONTAL | Gravity.CENTER_VERTICAL);
            ((ViewGroup)mListView.getParent()).addView(tipsView);
		}
	}
	 ...
	 * @param resId
     */
	public void setTipsContnet(int resId){

		if (mListView != null && tipsView == null){

			tipsView = (LinearLayout) LayoutInflater.from(getContext())
					.inflate(R.layout.pull_to_refresh_tips_content, null);
			((ImageView)tipsView.findViewById(R.id.img_no_data)).setImageResource(resId);
			LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(LinearLayout.LayoutParams.FILL_PARENT, LinearLayout.LayoutParams.FILL_PARENT);
			tipsView.setLayoutParams(params);
			tipsView.setGravity(Gravity.CENTER_HORIZONTAL | Gravity.CENTER_VERTICAL);
			((ViewGroup)mListView.getParent()).addView(tipsView);
	 ...
	 * 移除提示信息
	 */
	public void removeTipsContent(){
		if (tipsView != null) ((ViewGroup)mListView.getParent()).removeView(tipsView);
		tipsView = null;
	}
